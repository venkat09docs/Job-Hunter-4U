...
export const AdminGitHubReenableRequestsDialog: React.FC = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [requests, setRequests] = useState<GitHubReenableRequest[]>([]);
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      console.error('Error fetching GitHub extension requests:', error);
      toast.error('Failed to fetch GitHub extension requests');
      setRequests([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleApproveRequest = async (requestId: string, userTaskId: string, dueAt: string) => {
    ...
    try {
      ...
      console.error('Error approving GitHub extension request:', error);
      toast.error('Failed to approve GitHub extension request');
    } finally {
  ...
  const handleRejectRequest = async (requestId: string) => {
    ...
    try {
      ...
      toast.success('GitHub extension request rejected');
      ...
      console.error('Error rejecting GitHub extension request:', error);
      toast.error('Failed to reject GitHub extension request');
    } finally {
      setProcessingId(null);
    }
  };
        ...
        <ScrollArea className="max-h-[60vh]">
          {loading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p>No pending GitHub extension requests</p>
            </div>
          ) : (
            <div className="space-y-4">
              {requests.map((request) => {
                const canExtend = canAdminExtendTask(request.github_user_tasks.due_at);
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <Badge variant="outline" className="text-xs">
                            {request.github_user_tasks.github_tasks.code}
                          </Badge>
                        </div>
                        
                        <p className="text-sm font-medium text-primary mb-1">
                          {request.github_user_tasks.github_tasks.title}
                        </p>
                        
                        <div className="text-xs text-muted-foreground space-y-1">
                          <div>
                            Due: {format(new Date(request.github_user_tasks.due_at), 'MMM dd, yyyy h:mm a')}
                          </div>
                          <div>
                      ...
                      <div className="flex gap-2">
                        <Button
                          ...
                          onClick={() => handleApproveRequest(
                            ...
                            request.github_user_tasks.due_at
                          )}
