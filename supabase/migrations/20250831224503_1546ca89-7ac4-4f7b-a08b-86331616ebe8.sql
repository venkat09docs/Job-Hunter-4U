-- Phase 3: Admin Notifications, Advanced Scheduling & Analytics (Fixed)

-- Create notification analytics table (if not exists)
...
-- Create notification schedules table (if not exists)
...
-- Create admin notification triggers table (if not exists)
...
-- Create user notification settings table (if not exists)
...
CREATE INDEX IF NOT EXISTS idx_delivery_log_status ON notification_delivery_log(delivery_status, delivery_timestamp);
CREATE INDEX IF NOT EXISTS idx_delivery_log_user ON notification_delivery_log(user_id, delivery_method, delivery_status);

-- Enable RLS on new tables
DO $$
BEGIN
  -- Notification Analytics
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'notification_analytics') THEN
  ...
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'notification_schedules') THEN
  ...
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'admin_notification_triggers') THEN
