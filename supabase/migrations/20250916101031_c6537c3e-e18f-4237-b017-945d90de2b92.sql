-- Fix the auto_grade_attempt function to avoid ambiguous column references
...
-- Fix the trigger function to properly handle the auto-grading
CREATE OR REPLACE FUNCTION public.trigger_auto_submit()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
BEGIN
  -- Only auto-grade when attempt is submitted
  IF NEW.status = 'submitted' AND (OLD.status IS NULL OR OLD.status != 'submitted') THEN
    -- Call auto-grading function
...
DROP TRIGGER IF EXISTS trigger_auto_submit_attempts ON clp_attempts;
CREATE TRIGGER trigger_auto_submit_attempts
