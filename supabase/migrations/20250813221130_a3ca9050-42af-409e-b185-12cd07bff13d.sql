...
-- Using the correct approach for Supabase RLS

-- 1. Fix portfolios table - recreate policies to require authentication
...
-- 2. Fix profiles table - ensure only authenticated access
...
-- Recreate secure authenticated-only policies
...
-- Only allow users to view their own payments (authenticated only)
...
-- 4. Fix AI chat logs - ensure authenticated access only
...
-- 5. Fix resume data tables - ensure authenticated access only
...
-- 6. Fix saved resumes table - ensure authenticated access only
