...
CREATE POLICY IF NOT EXISTS "Users can view their own daily flow sessions"
ON public.github_daily_flow_sessions
FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own daily flow sessions"
ON public.github_daily_flow_sessions
FOR INSERT
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own daily flow sessions"
ON public.github_daily_flow_sessions
FOR UPDATE
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete their own daily flow sessions"
ON public.github_daily_flow_sessions
FOR DELETE
USING (auth.uid() = user_id);
