-- Fix the submit_assignment_direct function by removing the problematic ON CONFLICT clause
...
BEGIN
  ...
  -- Check if assignment_submissions record already exists, then insert or update
  IF EXISTS (SELECT 1 FROM public.assignment_submissions WHERE assignment_id = p_assignment_id AND user_id = p_user_id) THEN
