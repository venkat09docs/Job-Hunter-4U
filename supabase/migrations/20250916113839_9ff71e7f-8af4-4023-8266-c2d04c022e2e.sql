-- Fix the auto_grade_attempt function to use correct enum values
  ...
  LOOP
    ...
    IF question_record.kind = 'mcq' THEN
      ...
      IF answer_record.response IS NOT NULL THEN
        ...
        END;
      END IF;
    ELSIF question_record.kind = 'tf' THEN
      -- For True/False, check if the selected option matches the correct answer
      IF answer_record.response IS NOT NULL THEN
        DECLARE
          selected_option TEXT;
          correct_answers_array JSONB;
        ...
        END;
      END IF;
    ELSIF question_record.kind = 'descriptive' THEN
      -- For descriptive questions, mark as pending review (no auto-grading)
      UPDATE clp_answers 
