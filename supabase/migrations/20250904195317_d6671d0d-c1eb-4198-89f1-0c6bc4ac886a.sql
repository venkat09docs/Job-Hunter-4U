-- Create a new leaderboard function that handles both institute and non-institute users
CREATE OR REPLACE FUNCTION public.get_leaderboard_filtered(
  period_type text DEFAULT 'current_week',
  limit_count integer DEFAULT 5,
  is_institute_user boolean DEFAULT false,
  user_institute_id uuid DEFAULT NULL
)
RETURNS TABLE(
  user_id uuid,
  full_name text,
...
AS $$
  ...
  user_points AS (
    ...
    WHERE uap.activity_date >= dr.start_date 
      ...
      AND (
        -- If requesting user is NOT an institute user, exclude all institute users
        (is_institute_user = false AND NOT EXISTS (
          SELECT 1 
        ...
        OR
        -- If requesting user IS an institute user, only show users from same institute
        (is_institute_user = true AND user_institute_id IS NOT NULL AND EXISTS (
          SELECT 1 
          FROM public.user_assignments ua 
          WHERE ua.user_id = uap.user_id 
            AND ua.institute_id = user_institute_id
            AND ua.is_active = true
        ))
      )
