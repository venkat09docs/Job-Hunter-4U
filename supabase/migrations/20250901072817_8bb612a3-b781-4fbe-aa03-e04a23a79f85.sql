-- Drop all specific versions of create_smart_notification function with explicit signatures
DROP FUNCTION IF EXISTS public.create_smart_notification(uuid, text, jsonb, text, text, timestamp with time zone) CASCADE;
DROP FUNCTION IF EXISTS public.create_smart_notification(uuid, text, jsonb, text, timestamp with time zone, text) CASCADE;
DROP FUNCTION IF EXISTS public.create_smart_notification CASCADE;

-- Recreate the function with a single, clear signature
CREATE FUNCTION public.create_smart_notification(
  user_id_param uuid, 
