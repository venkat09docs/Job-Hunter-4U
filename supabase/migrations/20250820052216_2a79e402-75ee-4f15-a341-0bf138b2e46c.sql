...
-- Create function to send achievement notifications based on profile activity
...
BEGIN
  -- Check for resume achievements
  IF NEW.total_resume_opens >= 50 AND COALESCE(OLD.total_resume_opens, 0) < 50 THEN
  ...
  ELSIF NEW.total_ai_queries >= 200 AND COALESCE(OLD.total_ai_queries, 0) < 200 THEN
    achievement_title := 'Achievement Unlocked: AI Explorer!';
    achievement_message := 'Fantastic! You''ve made 200 AI queries. You''re leveraging technology for your career growth!';
  ELSE
