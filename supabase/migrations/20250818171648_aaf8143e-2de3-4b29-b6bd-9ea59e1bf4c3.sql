-- Fix search_path security issue by updating our functions
DROP FUNCTION IF EXISTS public.get_safe_admin_profiles(uuid[]);
DROP FUNCTION IF EXISTS public.get_safe_institute_profiles(uuid);
...
SET search_path = 'public'  -- Fixed: Set immutable search path
AS $$
  SELECT 
    p.user_id,
    p.full_name,
