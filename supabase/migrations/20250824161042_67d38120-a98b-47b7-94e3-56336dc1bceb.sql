-- Fix the create_payment_record function to handle all columns properly
...
BEGIN
  ...
  -- Insert new payment record with explicit column specification and proper defaults
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE LOG 'Error creating payment record: %, SQLSTATE: %', SQLERRM, SQLSTATE;
    RAISE;
END;
$function$;